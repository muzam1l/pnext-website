...
const CANDIDATES = ['node_modules/@wular/pnext/reference/data/bench.json', 'pnext/reference/data/bench.json', '../pnext/reference/data/bench.json'];
...
type BenchJson = {
  ...
  metrics: { id: string; label: string; category: Category; phase: Phase; rows: Partial<Record<FixtureId, Row5>> }[];
  headlines: { label: string; value: string; direction: 'faster' | 'less' | 'smaller'; detail?: string }[];
...
function validate(data: unknown): asserts data is BenchJson {
  ...
  for (const metric of d.metrics as Record<string, unknown>[]) {
    assert(typeof metric.id === 'string' && typeof metric.label === 'string', `metric missing id/label`);
    ...
    for (const [fixtureId, row] of Object.entries(metric.rows as Record<string, unknown>)) {
      assert(Array.isArray(row) && row.length === 5, `metric "${metric.id}" row "${fixtureId}" must be a 5-tuple`);
    }
  }
}
